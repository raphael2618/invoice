...
def upload_invoice(request):
    ...
    # 1. Cards de résumé (Mois en cours)
    current_month_invoices = user_invoices.filter(date__month=now.month, date__year=now.year)
    total_month = current_month_invoices.aggregate(Sum('total_ttc'))['total_ttc__sum'] or 0
    tva_month = current_month_invoices.aggregate(Sum('tva'))['tva__sum'] or 0
    count_month = current_month_invoices.count()
